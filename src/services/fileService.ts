...
export async function writeFile(
  ...
  if (append) {
    // Native append (RNFS "wa" mode: creates the file if missing, positions at
    // end). Replaces the old read-modify-write, which was O(n²) for large files
    // and froze the JS main thread (UI) during chunked downloads.
    const dir = path.substring(0, path.lastIndexOf("/") + 1);
    const exists = await RNFS.exists(dir);
    if (!exists) {
      await RNFS.mkdir(dir);
