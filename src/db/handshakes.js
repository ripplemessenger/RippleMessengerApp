...
export const api = {
  async initHandshakeFromLocal(
    ...
    await db.execute(
      ...
      [
        ...
        private_key,
        public_key,
        JSON.stringify(self_json),
      ],
    );
  },

  async initHandshakeFromRemote(
    self_address,
    pair_address,
    partition,
    sequence,
  ...
  /**
   * List all handshakes where self_address = address (for LAN sync push).
   * self_json/pair_json are kept as raw JSON strings (Client expects strings).
   */
  async getMyHandshakes(address) {
    const dbInstance = await getDB();
    const rows = await dbInstance.select(
      "SELECT * FROM handshakes WHERE self_address = $1 ORDER BY pair_address, partition, sequence",
      [address],
    );
    return rows;
  },
