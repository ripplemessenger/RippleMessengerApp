...
export const api = {
  ...
  /**
   * All follows including deleted (for LAN sync push).
   */
  async getAllFollows(address) {
    const dbInstance = await getDB();
    return await dbInstance.select(
  ...
  async deleteFollow(local, remote) {
    const db = await getDB();
    await db.execute(
      "UPDATE follows SET is_deleted = 1, updated_at = $3 WHERE local = $1 AND remote = $2",
      [local, remote, Date.now()],
    );
  },

  /**
   * Set follow deletion state (for LAN sync pull).
   */
  async setFollowDeleted(local, remote, isDeleted, timestamp) {
    const db = await getDB();
    await db.execute(
