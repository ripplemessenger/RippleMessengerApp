...
export function createMultiWsChannel(configs) {
  Logger.info(
    "[WS] createMultiWsChannel creating connections:",
    configs.length,
    "URLs:",
    ...
    function connect() {
      ...
      ws.onclose = (ev) => {
        ...
        if (shouldRetry) {
          ...
          Logger.info(
            `[WS] Reconnecting ${key} in ${manager.RETRY_DELAY / 1000}s (attempt ${retryCount})`,
          );
        } else if (retryCount >= manager.MAX_RETRIES) {
          Logger.warn(
            `[WS] Retries exhausted for ${key} -- pausing (will retry on next createMultiWsChannel call)`,
          );
          ws.onclose = null;
          ws.onerror = null;
          ws.onmessage = null;
