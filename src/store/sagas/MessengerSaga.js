...
export function* LoadServerList() {
  try {
    ...
    if (server_list.length === 0) {
      ...
      yield call(() =>
        dbAPI.toggleServerConnect(DefaultServer, true, timestamp),
      );
      server_list.push({
        key: DefaultServer,
        url: DefaultServer,
    ...
    Logger.info(
      "[LoadServerList] Server count:",
      server_list.length,
      "URLs:",
      server_list.map((s) => s.url).join(", "),
    );
...
export function* ServerToggle({ payload }) {
  try {
    yield call(() =>
      dbAPI.toggleServerConnect(payload.url, payload.is_connect, Date.now()),
    );
    yield call(LoadServerList);
  } catch (e) {
...
export function* LoadGroupList() {
  try {
    ...
    group_list = group_list.filter(
      (g) =>
        g.is_accepted === true &&
        (g.created_by === address || g.member.includes(address)),
    );
...
export function* AcceptGroupRequest({ payload }) {
  try {
    ...
    if (
      group_request !== null &&
      group_request.is_accepted === false &&
      (group_request.created_by === address ||
        group_request.member.includes(address))
    ) {
...
export function* RejectGroupRequest({ payload }) {
  try {
    ...
    if (
      ...
      yield call(() => dbAPI.rejectGroupRequest(payload.hash));
      yield call(LoadGroupRequestList);
    }
  } catch (e) {
    Logger.error("[RejectGroupRequest] failed:", e.message);
  }
}
