...
import { FLASH_DURATION_MS, BulletinPageSize, FileChunkSize, FileMaxSize, Hour } from '../../lib/AppConst'
...
import { SendMessage, genFileNonce, getFileRequestList, setFileRequestList, safeFork } from './messenger.core'
...
/**
 * PasteBulletin — validate and cache a raw bulletin JSON pasted by the user.
 * Validates schema (AJV) and signature (XRPL EdDSA), then stores via CacheBulletin.
 * Payload: { json_str: string }
 */
export function* PasteBulletin({ payload }) {
  try {
    const { checkBulletinSchema } = require('../../lib/MessageSchemaVerifier')
    const { VerifyJsonSignature } = require('../../lib/MessengerUtil')

    let parsed
    try {
      parsed = JSON.parse(payload.json_str)
    } catch (e) {
      yield put(setFlashNoticeMessage({ message: 'Invalid JSON — please check the format.', duration: FLASH_DURATION_MS }))
      return
    }

    // Validate against bulletin schema
    if (!checkBulletinSchema(parsed)) {
      yield put(setFlashNoticeMessage({ message: 'Bulletin schema validation failed.', duration: FLASH_DURATION_MS }))
      return
    }

    // Verify cryptographic signature
    if (!VerifyJsonSignature(parsed)) {
      yield put(setFlashNoticeMessage({ message: 'Signature verification failed — bulletin may be tampered.', duration: FLASH_DURATION_MS }))
      return
    }

    const result = yield call(CacheBulletin, parsed)
    if (result !== null) {
      yield put(setFlashNoticeMessage({ message: 'Bulletin imported successfully.', duration: FLASH_DURATION_MS }))
    } else {
      yield put(setFlashNoticeMessage({ message: 'Bulletin already exists or save failed.', duration: FLASH_DURATION_MS }))
    }
  } catch (e) {
    Logger.error('[PasteBulletin] failed:', e.message)
    yield put(setFlashNoticeMessage({ message: 'Failed to import bulletin.', duration: FLASH_DURATION_MS }))
  }
}
...
export function* LoadPortalBulletin({ payload }) {
  try {
    ...
    yield put(setPortalBulletinList({ List: bulletins, Page: page, TotalPage: totalPage }))
  } catch (e) {
...
export function* LoadAddressBulletin({ payload }) {
  try {
    ...
    const totalPage = Math.max(1, Math.ceil(total / BulletinPageSize))
    yield put(setAddressBulletinList({ List: bulletins, Page: payload.page, TotalPage: totalPage }))
  } catch (e) {
...
export function* LoadFollowBulletin({ payload }) {
  try {
    ...
    if (follow_list.length > 0) {
      ...
      const totalPage = Math.max(1, Math.ceil(total / BulletinPageSize))
      yield put(setFollowBulletinList({ List: bulletins, Page: payload.page, TotalPage: totalPage }))
    } else {
...
export function* LoadBookmarkBulletin({ payload }) {
  try {
    ...
    const totalPage = Math.max(1, Math.ceil(total / BulletinPageSize))
    yield put(setBookmarkBulletinList({ List: bulletins, Page: page, TotalPage: totalPage }))
  } catch (e) {
...
export function* RequestReplyBulletin({ payload }) {
  try {
    ...
    if (!connect_status) {
      ...
      yield put(setDisplayBulletinReplyList({ List: replys, Page: payload.page, TotalPage: total_page }))
    } else {
...
export function* RequestTagBulletin({ payload }) {
  try {
    ...
    if (!connect_status) {
      ...
      if (tag_ids.length === 0) {
        yield put(setTagBulletinList({ List: [], Page: payload.page, TotalPage: 1 }))
        return
      }
      ...
      yield put(setTagBulletinList({ List: bulletins, Page: payload.page, TotalPage: total_page }))
    } else {
...
export function* PublishBulletin(action) {
  try {
    const seed = yield select(state => state.User.Seed)
    if (!seed) {
      yield put(setFlashNoticeMessage({ message: 'Not logged in', duration: FLASH_DURATION_MS }))
      return
    }
    const address = yield select(state => state.User.Address)
    ...
    if (last_bulletin === null) {
      ...
      bulletin_json = yield call(
        () => mgAPI.genBulletinJson(seed, last_bulletin.sequence + 1, last_bulletin.hash, tag, quote, file, action.payload.content, timestamp)
      )
    }
    ...
    yield put(setFlashNoticeMessage({ message: 'Bulletin published successfully', duration: FLASH_DURATION_MS }))
  } catch (e) {
...
export function* BulletinTagAdd({ payload }) {
  try {
    ...
    yield put(setFlashNoticeMessage({ message: 'Failed to add tag', duration: FLASH_DURATION_MS }))
  }
}
...
export function* BulletinTagDel({ payload }) {
  try {
    ...
    yield put(setFlashNoticeMessage({ message: 'Failed to remove tag', duration: FLASH_DURATION_MS }))
  }
}
...
export function* BulletinQuoteAdd({ payload }) {
  try {
    ...
    yield put(setFlashNoticeMessage({ message: 'Failed to add quote', duration: FLASH_DURATION_MS }))
  }
}
...
export function* BulletinQuoteDel({ payload }) {
  try {
    ...
    yield put(setFlashNoticeMessage({ message: 'Failed to remove quote', duration: FLASH_DURATION_MS }))
  }
}
...
export function* BulletinReply(action) {
  try {
    ...
    yield call(BulletinQuoteAdd, { payload: { Hash: quoteHash, Size: 0, Name: '' } })
    ...
    const displayBulletin = yield select(state => state.Messenger.DisplayBulletin)
    if (displayBulletin && displayBulletin.hash === quoteHash) {
      yield fork(RequestReplyBulletin, { payload: { hash: quoteHash, page: 1 } })
    }

    yield put(setFlashNoticeMessage({ message: 'Reply published successfully', duration: FLASH_DURATION_MS }))
  } catch (e) {
    Logger.error('[BulletinReply] failed:', e.message)
    yield put(setFlashNoticeMessage({ message: 'Reply failed', duration: FLASH_DURATION_MS }))
  }
}
...
export function* BulletinQuote({ payload }) {
  try {
    ...
    yield put(setFlashNoticeMessage({ message: 'Failed to quote bulletin', duration: FLASH_DURATION_MS }))
  }
}
...
export function* BulletinFileAdd({ payload }) {
  try {
    ...
    if (fileInfo.size > FileMaxSize) {
      yield put(setFlashNoticeMessage({ message: `File too large (more than ${filesize_format(FileMaxSize)})`, duration: FLASH_DURATION_MS }))
      return
    }
    ...
    const currentPublishFiles = yield select(state => state.Messenger.PublishFileList)
    yield put(setPublishFileList([...(currentPublishFiles || []), fileRef]))
  } catch (e) {
    Logger.error('[BulletinFileAdd] failed:', e.message, e.stack)
    yield put(setFlashNoticeMessage({ message: 'Failed to add file attachment', duration: FLASH_DURATION_MS }))
  }
}
...
export function* BulletinFileDel({ payload }) {
  try {
    ...
    yield put(setFlashNoticeMessage({ message: 'Failed to remove file', duration: FLASH_DURATION_MS }))
  }
}
  ...
  try {
    ...
    if (bulletin_db !== null) {
      ...
      if (displayBulletin && displayBulletin.hash === payload.hash) {
        yield put(setDisplayBulletin({ ...displayBulletin, is_marked: newMarkState }))
      }

      // Update the reply list if it contains this bulletin
      const replyList = yield select(state => state.Messenger.DisplayBulletinReplyList)
      if (replyList && Array.isArray(replyList)) {
        const updatedReplies = replyList.map(b =>
          b.hash === payload.hash ? { ...b, is_marked: newMarkState } : b
        )
        const replyPage = yield select(state => state.Messenger.DisplayBulletinReplyPage)
        const replyTotalPage = yield select(state => state.Messenger.DisplayBulletinReplyTotalPage)
        yield put(setDisplayBulletinReplyList({ List: updatedReplies, Page: replyPage, TotalPage: replyTotalPage }))
      }

      // Update the portal list if it contains this bulletin
      const portalList = yield select(state => state.Messenger.PortalBulletinList)
      if (portalList && Array.isArray(portalList)) {
        const updatedPortal = portalList.map(b =>
          b.hash === payload.hash ? { ...b, is_marked: newMarkState } : b
        )
        const portalPage = yield select(state => state.Messenger.PortalBulletinPage)
        const portalTotalPage = yield select(state => state.Messenger.PortalBulletinTotalPage)
        yield put(setPortalBulletinList({ List: updatedPortal, Page: portalPage, TotalPage: portalTotalPage }))
      }
      ...
      const bookmarkList = yield select(state => state.Messenger.BookmarkBulletinList)
      if (bookmarkList && Array.isArray(bookmarkList)) {
        ...
        if (newMarkState) {
          ...
          updatedBookmarks = bookmarkList.filter(b => b.hash !== payload.hash)
          const total = yield call(() => dbAPI.getBulletinCountByIsmark())
          const totalPage = Math.max(1, Math.ceil(total / BulletinPageSize))
          yield put(setBookmarkBulletinList({ List: updatedBookmarks, Page: 1, TotalPage: totalPage }))
          return
        }
      }
    }
