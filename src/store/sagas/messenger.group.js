...
export function* RefreshGroupMessageList() {
  try {
    ...
    const raw_list = yield call(() =>
      dbAPI.getGroupSession(CurrentSession.hash),
    );
...
function* RequestGroupMessageSync({ payload }) {
  try {
    ...
    const latest_msg = yield call(() =>
      dbAPI.getLastGroupMessage(payload.hash),
    );
    if (latest_msg) {
      ...
      yield call(SendMessage, {
        msg: JSON.stringify(group_message_sync_request),
      });
    } else {
      const address = yield select((state) => state.User.Address);
      const group_message_sync_request = yield call(() =>
        mgAPI.genGroupMessageSync(seed, payload.hash, address, 0),
      );
      yield call(SendMessage, {
        msg: JSON.stringify(group_message_sync_request),
      });
...
export function* LoadGroupSession({ payload }) {
  try {
    ...
    if (group_session.member.includes(self_address)) {
      const current_group_msg = yield call(() =>
        dbAPI.getMemberLastGroupMessage(payload.hash, self_address),
      );
      ...
      yield call(RequestGroupMessageSync, {
        payload: { hash: group_session.hash },
      });
    }
  } catch (e) {
...
function* SendGroupMessageToMember(
  group_hash,
  self_address,
  member,
  msg_json,
  ...
  try {
    ...
    const ecdh = yield call(() =>
      dbAPI.getHandshake(self_address, member, DefaultPartition, ecdh_sequence),
    );
    if (ecdh === null) {
...
export function* SendGroupContent({ payload }) {
  try {
    ...
    const cleared = yield call(() => dbAPI.isGroupCleared(CurrentSession.hash));
    if (cleared) {
      Logger.info("[SendGroupMessage] Group is cleared, skipping send");
      return;
    }
...
export function* ComposeMemberAdd({ payload }) {
  try {
    const address = yield select((state) => state.User.Address);
    const old_list = yield select(
      (state) => state.Messenger.ComposeMemberList,
    ) || [];
...
export function* CreateGroup(action) {
  try {
    ...
    let json = yield call(() =>
      mgAPI.genGroupCreate(seed, hash, action.payload.name, member),
    );
...
export function* DeleteGroup(action) {
  try {
    ...
    if (
      ...
      let json = yield call(() =>
        mgAPI.genGroupDelete(seed, action.payload.hash),
      );
      let result = yield call(() =>
        dbAPI.updateGroupDelete(action.payload.hash, json),
      );
      if (result > 0) {
        let group_response = {
          ObjectType: ObjectType.GroupList,
...
export function* ClearGroupData(action) {
  try {
    const hash = action.payload.hash;
    const cleared = yield call(() => dbAPI.isGroupCleared(hash));
    if (cleared) return;
    yield call(() => dbAPI.clearGroupData(hash));
    yield call(LoadSessionList);
    yield call(LoadGroupList);
  } catch (e) {
    Logger.error("[ClearGroupData] failed:", e.message);
  }
}
...
export function* GroupSync(payload) {
  try {
    ...
    yield call(SendMessage, {
      key: payload.key,
      msg: JSON.stringify(group_sync_request),
    });
  } catch (e) {
