...
export function* ForwardBulletin({ payload }) {
  try {
    ...
    yield put(setFlashNoticeMessage({ message: 'Bulletin forwarded successfully', duration: FLASH_DURATION_MS }))
  } catch (e) {
    Logger.error('[ForwardBulletin] failed:', e.message)
    yield put(setFlashNoticeMessage({ message: 'Failed to forward bulletin', duration: FLASH_DURATION_MS }))
  }
}
