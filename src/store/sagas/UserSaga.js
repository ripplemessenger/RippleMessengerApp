  ...
  try {
    ...
    if (contact_list.length === 0) {
      yield call(addContact, {
        payload: { address: PostAddress, nickname: "London" },
      });
      yield call(toggleContactFollow, {
        payload: { contact_address: PostAddress },
      });
    }
    ...
    if (!payload.isTemp) {
      yield call(saveSession, payload.seed, payload.address);
    }
  } catch (e) {
...
function* addAccount({ payload }) {
  try {
    let local_account = yield call(() =>
      dbAPI.getAccountByAddress(payload.address),
    );
...
function* loadContactList() {
  try {
    ...
    yield put(
      setContactList({ contact_list: contact_list, contact_map: contact_map }),
    );
...
function* toggleContactFriend({ payload }) {
  try {
    const address = yield select((state) => state.User.Address);
    const session_list_old = yield select(
      (state) => state.Messenger.SessionList,
    );
    const contact_address = payload.contact_address;
    ...
    if (friend !== null) {
      ...
      yield put(
        setSessionList(
          session_list_old.filter((s) => s.address !== contact_address),
        ),
      );
    } else {
...
export function* watchUser() {
  yield takeLatest(autoLoginInit.type, handleAutoLogin);
